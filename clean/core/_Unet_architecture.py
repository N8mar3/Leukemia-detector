...
import torch
# https://github.com/johschmidt42/PyTorch-2D-3D-UNet-Tutorial/blob/master/unet.py
...
class DownBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # conv layers
        self.conv1 = get_conv_layer(self.in_channels, self.out_channels, kernel_size=3,
                                    stride=1, padding=self.padding, bias=True, dim=self.dim)
        self.conv2 = get_conv_layer(self.out_channels, self.out_channels, kernel_size=3,
                                    stride=1, padding=self.padding, bias=True, dim=self.dim)
...
class UpBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.up = get_up_layer(self.in_channels, self.out_channels, kernel_size=2,
                               stride=2, dim=self.dim, up_mode=self.up_mode)

        # conv layers
        self.conv0 = get_conv_layer(self.in_channels, self.out_channels, kernel_size=1,
...
class UNet(nn.Module):
    def __init__(self,
        ...
        # final convolution
        self.conv_final = get_conv_layer(num_filters_out, self.out_channels,
