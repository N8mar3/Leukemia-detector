...
class Net(nn.Module):
    def __init__(self, threshold=10.0, use_cuda=False):
        ...
        self.gaussian_filter_horizontal = nn.Conv2d(in_channels=1, out_channels=1,
                                                    kernel_size=(1, filter_size),
                                                    padding=(0, filter_size//2))
        self.gaussian_filter_horizontal.weight.data.copy_(torch.from_numpy(generated_filters))
        self.gaussian_filter_horizontal.bias.data.copy_(torch.from_numpy(np.array([0.0])))
        self.gaussian_filter_vertical = nn.Conv2d(in_channels=1, out_channels=1,
    ...
    def forward(self, img):
        ...
        if self.use_cuda:
            pixel_range = torch.FloatTensor([range(pixel_count)]).cuda()
        ...
        assert grad_mag.size() == grad_orientation.size() ==\
               thin_edges.size() == thresholded.size() ==\
